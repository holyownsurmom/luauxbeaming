  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl brutal-border bg-card p-8 flex items-center justify-between gap-6 glow-primary">
        ...
        <div className="hidden md:flex flex-col items-end">
          ...
          <div className="mt-3 text-right text-xs">
            <div className="text-muted-foreground uppercase tracking-widest text-[10px]">Runtime</div>
            <div className="font-mono text-lg">{botHours.toFixed(1)}h</div>
            <div className="mt-1 text-muted-foreground uppercase tracking-widest text-[10px]">Expires</div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={BotIcon} label="Max Bots" value={String(maxBots)} hint={maxBots === 0 ? "no plan" : "concurrent slots"} locked={!active} />
        <StatCard icon={Clock} label="Bot Hours" value={`${botHours.toFixed(1)}h`} hint={active ? "included with plan" : "no plan"} locked={!active} />
        <StatCard icon={Calendar} label="Days Left" value={String(daysLeft)} hint={active ? "until renewal" : "no plan"} locked={!active} />
        <StatCard icon={Puzzle} label="Plugins" value="0" hint="owned" />
      </section>

      {/* Quick actions */}
      <section className="grid md:grid-cols-2 gap-4">
        <QuickAction to="/dashboard/purchase" icon={ShoppingCart} title="Get a plan" desc="Pay with LTC, SOL, or USDT/USDC. Access unlocks after 2 confirmations." cta="Choose plan" />
          <QuickAction to="/dashboard/bots" icon={BotIcon} title="MC Auto-Message" desc="Join any Minecraft server and auto-message with your bot fleet." cta="Configure & launch" />
      </section>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <PluginCard to="/dashboard/discord-spam" icon={Zap} title="Discord Auto-Spam" desc="Multi-token channel spammer with rotation, humanization & live console." />
          <PluginCard to="/dashboard/discord-bot" icon={Puzzle} title="More Plugins" desc="Browse verification bot, auto-reply, and more Discord automation tools." />
        </div>
      </section>
    </div>
  );
...
function StatCard({
  ...
  return (
    <div className={`rounded-2xl brutal-border bg-card p-5 relative overflow-hidden ${locked ? "opacity-70" : ""}`}>
...
function QuickAction({
  ...
  return (
    <Link to={to} className="rounded-2xl brutal-border bg-card p-5 flex items-start gap-4 hover:bg-card/80 transition-colors">
...
function PluginCard({ icon: Icon, title, desc, to }: { icon: React.ComponentType<{ className?: string }>; title: string; desc: string; to: string }) {
  return (
    <Link to={to} className="rounded-2xl brutal-border bg-card p-5 hover:bg-card/80 transition-colors block">
...
function RuntimeRing({ daysLeft }: { daysLeft: number }) {
  ...
  return (
    <svg width="88" height="88" viewBox="0 0 88 88">
      <circle cx="44" cy="44" r={r} fill="none" stroke="currentColor" className="text-secondary" strokeWidth="6" />
      <circle cx="44" cy="44" r={r} fill="none" stroke="currentColor" className="text-primary" strokeWidth="6" strokeDasharray={c} strokeDashoffset={c * (1 - pct)} strokeLinecap="round" transform="rotate(-90 44 44)" />
      <text x="44" y="42" textAnchor="middle" className="fill-foreground font-mono" fontSize="18" fontWeight="600">{daysLeft}</text>
      <text x="44" y="58" textAnchor="middle" className="fill-muted-foreground" fontSize="8" letterSpacing="2">DAYS LEFT</text>
    </svg>
  );
}